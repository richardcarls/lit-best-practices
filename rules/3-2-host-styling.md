...
**Custom element best practice:** Always give the host a sensible default display value and always
restore native hidden behavior with `:host([hidden]) { display: none; }`. Do not overwrite
author-owned `class` or `style` attributes from component code.
